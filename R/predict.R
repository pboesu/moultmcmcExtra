...
#' @export
#'
predict_ranef <- function(object, newdata = NULL, predict.type = "parameters", summary = TRUE, intervals = 0.1, prob = 0.95, ...){
  ...
  #get vrandom intercepts - NB the names for these are currently hard coded in the stan model irrespective of the relevant column name
  u_mu <- do.call(cbind, rstan::extract(object$stanfit, pars = names(object$stanfit)[grepl('u_year_mean', names(object$stanfit))]))
  u_tau <- do.call(cbind, rstan::extract(object$stanfit, pars = names(object$stanfit)[grepl('u_year_duration', names(object$stanfit))]))
